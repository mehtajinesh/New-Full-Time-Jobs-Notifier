...
from constants import COMPANY_NAMES_CSV, \
    ...
    COMPANY_KNOWN_JOBS_CSV, SLACK_DEPLOYMENT_NOTIFICATION_WEBHOOK, \
    SLACK_ERROR_NOTIFICATION_WEBHOOK, SLACK_JOB_NOTIFICATION_WEBHOOK
...
def send_deployment_notification_to_user(notification_type,
                                         notification_message, session):
    req = session.post(url=SLACK_DEPLOYMENT_NOTIFICATION_WEBHOOK,
                       headers={
...
def send_error_notification_to_user(notification_message, session):
    req = session.post(url=SLACK_ERROR_NOTIFICATION_WEBHOOK,
                       headers={
                           'Content-type': 'application/json'},
...
def send_notification_to_user(company_name, job_id, job_title, job_posted_date, job_application_link, session):
    req = session.post(url=SLACK_JOB_NOTIFICATION_WEBHOOK,
                       headers={
                           'Content-type': 'application/json'},
                       json={
...
def update_known_jobs(company_info):
        ...
        try:
            ...
            for company_id in company_info:
                ...
                relevant_jobs = get_relevant_jobs(
                    company_name, search_api_type, search_api_url, keywords, search_api_header, session)
                ...
                for job_id in relevant_jobs:
                    ...
                    if job_id not in known_jobs:
                        ...
                        send_notification_to_user(
                            company_name, job_id, job_title, job_posted_date, job_application_link, session)
